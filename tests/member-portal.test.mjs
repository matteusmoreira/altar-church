...
test("member ministry agenda exposes description, confirmed people and tenant-scoped RSVP", () => {
  const data = read("src/lib/member/data.ts")
  const portal = read("src/components/member/member-agenda.tsx")
  const actions = read("src/lib/member/portal-actions.ts")
  const migration = read("supabase/migrations/20260811150000_ministry_member_agenda_rsvp.sql")
  assert.match(data, /confirmed_people/)
  assert.match(data, /person\.full_name/)
  ...
  assert.match(migration, /programming_id is not null/)
  assert.match(migration, /registration_enabled = true/)
})
