...
test("self-service registration lets the member choose an active church", () => {
  ...
  assert.match(registerAction, /login_phone/)
  assert.match(registerPage, /id="whatsapp"/)
  assert.match(registerPage, /formatBrazilianWhatsapp/)
  assert.match(registerPage, /const \{ login \} = useAuth\(\)/)
  assert.match(registerPage, /await login\(email\.trim\(\)\.toLowerCase\(\), password\)/)
  assert.match(registerPage, /router\.replace\("\/dashboard"\)/)
  assert.doesNotMatch(registerPage, /toast\.success\("Conta criada\. Faça login para continuar\."\)/)
