...
test("PWA manifest has stable identity, adaptive entry and dedicated icons", () => {
  ...
  assert.match(entry, /redirect\("\/login\?next=\/"\)/)
  assert.match(entry, /user\.role === "guardian"[\s\S]*redirect\("\/familia\/kids"\)/)
  assert.match(entry, /user\.role === "volunteer"[\s\S]*redirect\("\/voluntariado"\)/)
