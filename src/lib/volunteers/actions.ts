...
const volunteerSchema = z.object({
  ...
  email: z.union([z.string().trim().email("E-mail inválido"), z.literal("")]).transform((value) => value || null),
  phone: z.string().trim().optional().default(""),
  registrationStatus: status.default("pending"),
  whatsappEnabled: z.boolean().default(false),
  emailEnabled: z.boolean().default(false),
  memberships: z.array(z.object({ departmentId: uuid, roleName: z.string().trim().min(1).max(80) })).default([]),
...
const scheduleSchema = z.object({ month: z.string().regex(/^\d{4}-\d{2}-01$/, "Mês inválido") })
const assignmentSchema = z.object({ shiftId: uuid, volunteerId: uuid, status: z.enum(["assigned", "confirmed", "declined", "cancelled"]).default("assigned") })
...
const checkinSchema = z.object({ assignmentId: uuid, qrToken: z.union([uuid, z.literal("")]).optional() })
...
async function assertDepartments(companyId: string, ids: string[]) {
  ...
  `
  if (rows.length !== new Set(ids).size) throw new Error("Departamento inválido")
}

async function inviteVolunteer(input: { companyId: string; personId: string; name: string; email: string; actorId: string }) {
  const sql = getSql()
  const existingRows = await sql<{ id: string; person_id: string | null; role: string; auth_user_id: string | null }[]>`
  ...
  if (existing?.person_id && existing.person_id !== input.personId) throw new Error("E-mail já vinculado a outra pessoa")
  ...
  if (!supabase) throw new Error("SUPABASE_SERVICE_ROLE_KEY obrigatória para convite")
  const invited = await supabase.auth.admin.inviteUserByEmail(input.email, {
    data: { name: input.name, role: existing?.role ?? "volunteer", company_id: input.companyId },
  })
  if (invited.error || !invited.data.user?.id) throw new Error(`Convite Auth falhou: ${invited.error?.message ?? "sem usuário"}`)
  await sql`
...
export async function saveVolunteer(input: z.input<typeof volunteerSchema>): Promise<VolunteerActionResult> {
  try {
    const parsed = volunteerSchema.parse(input)
    const { user, companyId } = await context(parsed.id ? "volunteers.edit" : "volunteers.create")
    await assertDepartments(companyId, parsed.memberships.map((membership) => membership.departmentId))
    const sql = getSql()
    ...
    await sql.begin(async (tx) => {
      ...
      if (!volunteerId || !personId) throw new Error("Voluntário não foi salvo")
    ...
    if (parsed.invite) {
      if (!parsed.email || !personId) throw new Error("E-mail obrigatório para convite")
      await requirePermission("volunteers.invite", companyId)
      await inviteVolunteer({ companyId, personId, name: fullName, email: parsed.email, actorId: user.id })
    }
    await audit("volunteer.save", "volunteer_profiles", volunteerId ?? "", companyId, { invite: parsed.invite })
    refresh()
...
export async function saveVolunteerDepartment(input: z.input<typeof departmentSchema>): Promise<VolunteerActionResult> {
  try {
    ...
    await audit("volunteer_department.save", "volunteer_departments", rows[0].id, companyId)
    refresh()
...
export async function saveVolunteerTemplate(input: z.input<typeof templateSchema>): Promise<VolunteerActionResult> {
  try {
    const parsed = templateSchema.parse(input)
    const { user, companyId } = await context(parsed.id ? "schedules.edit" : "schedules.create")
    await assertDepartments(companyId, parsed.slots.map((slot) => slot.departmentId))
    const sql = getSql()
    ...
    await audit("volunteer_template.save", "volunteer_schedule_templates", templateId ?? "", companyId)
    refresh()
...
export async function generateMonthlyVolunteerSchedule(input: z.input<typeof scheduleSchema>): Promise<VolunteerActionResult> {
  try {
    const parsed = scheduleSchema.parse(input)
    const { user, companyId } = await context("schedules.create")
    const sql = getSql()
    const events = await sql<{ id: string; volunteer_template_id: string; starts_at: Date; ends_at: Date | null }[]>`
    ...
    for (const event of events) {
      const slots = await sql<{ id: string; department_id: string; role_name: string; required_volunteers: number }[]>`
      ...
      for (const slot of slots) {
        ...
        const endsAt = event.ends_at ?? new Date(startsAt.getTime() + 2 * 60 * 60 * 1000)
    ...
    await audit("volunteer_schedule.generate", "volunteer_schedules", scheduleId, companyId, { month: parsed.month, events: events.length })
    refresh()
...
export async function saveVolunteerAssignment(input: z.input<typeof assignmentSchema>): Promise<VolunteerActionResult> {
  try {
    const parsed = assignmentSchema.parse(input)
    const { user, companyId } = await context("schedules.edit")
    const sql = getSql()
    const checks = await sql<{ shift_exists: boolean; volunteer_exists: boolean; available: boolean; already_assigned: boolean }[]>`
      select
    ...
    if (!check?.shift_exists || !check.volunteer_exists) throw new Error("Escala ou voluntário inválido")
    if (!check.available && !check.already_assigned) throw new Error("Todas as vagas desta função já foram preenchidas")
    const rows = await sql<{ id: string }[]>`
      insert into public.volunteer_assignments (company_id, shift_id, volunteer_id, status, created_by, updated_by)
      values (${companyId}, ${parsed.shiftId}, ${parsed.volunteerId}, ${parsed.status}, ${user.id}, ${user.id})
      on conflict (shift_id, volunteer_id) do update set status = excluded.status, updated_by = excluded.updated_by, updated_at = now()
      returning id
    `
    await audit("volunteer_assignment.save", "volunteer_assignments", rows[0].id, companyId)
    refresh()
    return { ok: true, id: rows[0].id }
  } catch (error) {
    return failure(error)
...
export async function publishVolunteerSchedule(scheduleId: string): Promise<VolunteerActionResult> {
  try {
    ...
    const recipients = await sql<{ assignment_id: string; volunteer_id: string; email: string | null; phone: string; email_enabled: boolean; whatsapp_enabled: boolean; event_title: string; starts_at: Date }[]>`
      select assignment.id as assignment_id, volunteer.id as volunteer_id, person.email, person.phone,
             volunteer.email_enabled, volunteer.whatsapp_enabled, coalesce(event.title, 'Escala') as event_title, shift.starts_at
    ...
    await audit("volunteer_schedule.publish", "volunteer_schedules", id, companyId, { recipients: recipients.length })
    refresh()
    return { ok: true, id }
  } catch (error) {
    return failure(error)
...
export async function saveVolunteerFeedPost(input: z.input<typeof feedSchema>): Promise<VolunteerActionResult> {
  try {
    const parsed = feedSchema.parse(input)
    if (parsed.audience === "departments" && parsed.departmentIds.length === 0) throw new Error("Selecione departamentos")
    const { user, companyId } = await context(parsed.publish ? "volunteer_feed.publish" : "volunteer_feed.create")
    ...
    if (parsed.publish && postId) {
      const recipients = await sql<{ id: string; email: string | null; phone: string; email_enabled: boolean; whatsapp_enabled: boolean }[]>`
      ...
      `
      for (const recipient of recipients) {
        if (recipient.whatsapp_enabled && recipient.phone) await sql`
        ...
        `
        if (recipient.email_enabled && recipient.email) await sql`
          insert into public.volunteer_delivery_outbox (company_id, volunteer_id, feed_post_id, channel, recipient, subject, content)
    ...
    await audit("volunteer_feed.save", "volunteer_feed_posts", postId ?? "", companyId, { published: parsed.publish })
    refresh()
...
export async function createVolunteerCheckinQr(shiftId: string): Promise<VolunteerActionResult> {
  try {
...
export async function checkInVolunteerAssignment(input: z.input<typeof checkinSchema>): Promise<VolunteerActionResult> {
  try {
    ...
    `
    await audit("volunteer_checkin.create", "volunteer_assignments", assignment.id, user.churchId, { source })
    refresh()
...
export async function markVolunteerFeedRead(postId: string): Promise<VolunteerActionResult> {
  try {
