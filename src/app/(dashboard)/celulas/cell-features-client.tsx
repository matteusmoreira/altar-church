...
function CellCheckboxes({ cells }: { cells: CellFeaturesData["cells"] }) {
  return <div className="grid max-h-40 gap-2 overflow-y-auto rounded-lg border p-3 sm:grid-cols-2">
    {cells.map((cell) => <label key={cell.id} className="flex items-center gap-2 text-sm"><input type="checkbox" name="groupIds" value={cell.id} defaultChecked className="h-4 w-4" />{cell.name}</label>)}
  </div>
}
