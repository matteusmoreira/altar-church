...
import { BookOpen, Camera, CheckCircle2, ClipboardCheck, Download, Heart, ImageIcon, Megaphone, QrCode, Upload } from "lucide-react"
...
export function CellFeaturesClient({ data }: { data: CellFeaturesData }) {
  ...
  const summaryCellId = selectedMeeting?.groupId ?? data.cells[0]?.id ?? ""
  const summaryCell = data.cells.find((cell) => cell.id === summaryCellId)
  const summaryMeetings = data.meetings.filter((meeting) => meeting.groupId === summaryCellId)
  const summaryMeetingIds = new Set(summaryMeetings.map((meeting) => meeting.id))
  const summaryAttendance = data.attendance.filter((item) => summaryMeetingIds.has(item.meetingId))
  const summaryVisitors = summaryAttendance.filter((item) => item.visitor).length
  const summaryQrCheckins = summaryAttendance.filter((item) => item.source === "qr").length
  ...
  return (
    <Card className="glass"><CardHeader><CardTitle>Operação avançada de Células</CardTitle><CardDescription>Estudos, QR, presença, mural, oração e avisos.</CardDescription></CardHeader><CardContent>
      <Tabs defaultValue="estudos">
        ...
        <TabsContent value="checkin" className="space-y-6">
          {data.meetings.length === 0 ? <Card><CardContent className="py-10 text-center text-muted-foreground">Nenhum encontro disponível. Crie um encontro em “Participantes e reuniões”; o estudo é opcional.</CardContent></Card> : <>
            ...
            {summaryCell && <Card className="border-primary/20 bg-primary/5"><CardHeader className="flex flex-row items-start justify-between gap-3"><div><CardTitle>Resumo geral da célula</CardTitle><CardDescription>{summaryCell.name} · histórico dos encontros registrados</CardDescription></div><Button type="button" variant="outline" size="sm" onClick={() => setSummaryOpen(true)}>Ver resumo completo</Button></CardHeader><CardContent className="grid grid-cols-2 gap-3 sm:grid-cols-4"><div><p className="text-2xl font-bold">{summaryMeetings.length}</p><p className="text-xs text-muted-foreground">Encontros</p></div><div><p className="text-2xl font-bold">{summaryAttendance.length}</p><p className="text-xs text-muted-foreground">Check-ins</p></div><div><p className="text-2xl font-bold">{summaryVisitors}</p><p className="text-xs text-muted-foreground">Visitantes</p></div><div><p className="text-2xl font-bold">{summaryQrCheckins}</p><p className="text-xs text-muted-foreground">Via QR</p></div></CardContent></Card>}
            ...
            <Dialog open={summaryOpen} onOpenChange={setSummaryOpen}><DialogContent className="sm:max-w-3xl"><DialogHeader><DialogTitle>Resumo completo · {summaryCell?.name}</DialogTitle><DialogDescription>Presenças, visitantes, origem e horário de cada check-in.</DialogDescription></DialogHeader><div className="space-y-5">{summaryMeetings.map((meeting) => { const meetingAttendance = summaryAttendance.filter((item) => item.meetingId === meeting.id); return <section key={meeting.id} className="space-y-3 rounded-xl border p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="font-semibold">{meeting.title}</h3><p className="text-sm text-muted-foreground">{dateTime(meeting.startsAt)}</p></div><Badge variant="secondary">{meetingAttendance.length} presentes</Badge></div>{meetingAttendance.length ? <div className="divide-y rounded-lg border">{meetingAttendance.map((item) => <div key={item.id} className="flex flex-wrap items-center justify-between gap-3 p-3 text-sm"><div><p className="font-medium">{item.personName}{item.visitor ? " · Visitante" : ""}</p><p className="text-xs text-muted-foreground">{dateTime(item.occurredAt)}</p></div><Badge variant="outline">{item.source === "qr" ? "QR" : "Manual"}</Badge></div>)}</div> : <p className="text-sm text-muted-foreground">Nenhum check-in neste encontro.</p>}</section> })}</div></DialogContent></Dialog>
          </>}
        </TabsContent>
