...
import type { CongregationListFilters } from "@/lib/congregations/types"
...
export default async function CongregationsPage({
  ...
  const filters: CongregationListFilters = {
  ...
  const congregationsResult = await listCongregations(filters)

  return (
    <CongregationsClient
      congregationsResult={congregationsResult}
      filters={filters}
    />
  )
}
