      ...
      <Card className="glass">
        ...
        <CardContent className="space-y-3">
          {channels.map(({ key, label, description, icon: Icon }) => {
            ...
            return (
              <form action={action} key={key} className="surface flex items-center justify-between gap-4 p-4">
                ...
                <div className="flex shrink-0 items-center gap-3"><label className="flex items-center gap-2 text-sm"><span className="sr-only">Bloquear {label}</span><input type="checkbox" name="optedOut" defaultChecked={preferences[key]} className="h-4 w-4 rounded border-border" /> Bloquear</label><Button type="submit" variant="outline" size="sm">Salvar</Button></div>
              </form>
            )
          })}
        </CardContent>
