...
export async function GET(request: NextRequest) {
  try {
    ...
    const healthRows = await listCellHealth(companyId)
    const rows = [
      ["Célula", "Saúde", "Membros ativos", "Capacidade", "Presença 7d", "Presença 30d", "Novos 30d", "Orações abertas", "Relatórios pendentes"],
      ...healthRows.map((row) => [row.name, row.health, row.activeMemberCount, row.capacity, row.attendance7, row.attendance30, row.newParticipants30, row.openPrayerCount, row.pendingReports30]),
    ] satisfies CsvCell[][]
