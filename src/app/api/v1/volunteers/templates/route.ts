...
import { saveVolunteerTemplate } from "@/lib/volunteers/actions"
...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await listVolunteerTemplatesForEvents(companyId)
    return jsonOk(data)
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    ...
    const result = await saveVolunteerTemplate(body as Parameters<typeof saveVolunteerTemplate>[0])
